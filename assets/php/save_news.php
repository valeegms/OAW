...
function save_news ($url, $feed_id){
		...
		foreach ($feed->get_items() as $feedItem) {
			...
			$itemImageURL = returnScrapedImage_($content);
			if ($itemImageURL === ""){
			...
			$itemText = returnText_($content);

			//$itemDate = date("Y-m-d H:i:s",strtotime($feedItem->get_local_date()));
			...
			echo $itemDate;
			$sql = "INSERT INTO news(feed, image_url, category,text,date,link) 
			VALUES ('" . $feed_id . "','" . $itemImageURL . "', '" . $categoryLabel . "', '" . $itemText ."','" . $itemDate . "','" . $itemPermalink ."')";
	...
	function returnScrapedImage_ ($text) {
